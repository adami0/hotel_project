...
const addClient = (clbk, data) => {
  const q = `INSERT INTO
                client (id_client, last_name, first_name, adress, phone_nb, email)
            VALUES
                (
...
const updateClient = (clbk, data, id) => {
  const q = `UPDATE
            ...
            WHERE
                id_client=${mysql.escape(id)}`;

  mysql.query(q, (error, result, fields) => {
    if (error) throw error;
    result.message = 'Les informations client ont été modifiées avec succès';
    clbk(result);
  });
};
...
const removeClient = (clbk, id) => {
  ...
  mysql.query(q, (error, result, fields) => {
      if (error) throw error;
      result.message = 'Votre client a été supprimé. RIP ฅ^•ﻌ•^ฅ';
      clbk(result);
  });
};
