...
const addReservation = (clbk, data) => {
  const q = `INSERT INTO reservation
    (arrival_date, departure_date, guests, price, nights, fk_id_rooms, fk_id_client)
    VALUES
    (
...
const updateReservation = (clbk, data, id) => {
  const q = `UPDATE reservation SET
    ...
    fk_id_client=${mysql.escape(data.id_client)} WHERE id_reservation=${mysql.escape(id)}`;
